    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-white">Quản lý Nhà sản xuất</h1>
                    <p className="text-gray-500">Quản lý danh sách các thương hiệu & hãng sản xuất (HP, Dell, IBM...).</p>
                </div>
                <button 
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {loading ? (
                            ...
                            <tr key={manufacturer.id} className="hover:bg-white/[0.02] transition-colors group">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-500">
                                            {manufacturer.logoUrl ? (
                                                <img src={manufacturer.logoUrl} alt={manufacturer.name} className="w-6 h-6 object-contain" />
                                            ) : (
                                                <Building2 className="w-5 h-5" />
                                            )}
                                        </div>
                                        <span className="text-sm font-medium text-white">{manufacturer.name}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 text-sm text-gray-400">{manufacturer.description || '-'}</td>
                    ...
                    <div className="bg-[#0d121f] border border-white/10 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between">
                            <h2 className="text-xl font-bold text-white">
                                {editingManufacturer ? 'Chỉnh sửa Nhà sản xuất' : 'Thêm Nhà sản xuất mới'}
                            </h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-gray-500 uppercase">Tên nhà sản xuất *</label>
                                    <input 
                                        required
                                        type="text" 
                                        value={formData.name}
                                        ...
                                        placeholder="VD: Hewlett-Packard"
                                    />
                                </div>

                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-gray-500 uppercase">Logo URL (tùy chọn)</label>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <textarea 
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20" 
                                        placeholder="Thông tin thêm về nhà sản xuất..."
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4 pt-4">
                                ...
                                >
                                    {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : (editingManufacturer ? 'Cập nhật' : 'Tạo mới')}
                                </button>
                            </div>
                        </form>
                    </div>
