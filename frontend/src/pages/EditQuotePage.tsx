...
const EditQuotePage = () => {
      ...
      try {
        ...
        if (id) {
          ...
          if (quote.status !== 'DRAFT') {
            setError('Chỉ có thể chỉnh sửa báo giá ở trạng thái Nháp (DRAFT).');
          } else {
            setSelectedCustomerId(quote.customerId);
            setExpiryDate(quote.expiryDate ? new Date(quote.expiryDate).toISOString().split('T')[0] : '');
            setApprovalDeadline(quote.approvalDeadline ? new Date(quote.approvalDeadline).toISOString().split('T')[0] : '');
            setPurchasingDeadline(quote.purchasingDeadline ? new Date(quote.purchasingDeadline).toISOString().split('T')[0] : '');
            
            if (quote.items && quote.items.length > 0) {
              setItems(quote.items.map((i: any, index: number) => ({
                ...
                leadTime: i.product.leadTime || '',
                priceExpiry: i.priceExpiry || i.product.priceExpiry,
              })));
            } else {
              setItems([]);
            }
        ...
        setError(err.response?.data?.message || 'Không thể tải dữ liệu báo giá.');
      } finally {
  ...
  if (initialLoading) {
    return <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 flex-shrink-0 animate-spin text-blue-500" /></div>;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-bold text-white">Sửa Báo giá</h1>
        <p className="text-gray-500">Chỉnh sửa thông tin yêu cầu báo giá Nháp.</p>
      </div>
